window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "fork",
          "formpag",
          "formsgithub",
          "github",
          "index",
          "pagin",
          "readm",
          "repo",
          "simpl",
          "way"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "undefined": [
          "event",
          "handl",
          "tutori"
        ],
        "list_tutorial.html": [
          "avail",
          "list",
          "list:tutori",
          "tutori"
        ],
        "list_class.html": [
          "class",
          "document",
          "list",
          "list:class"
        ],
        "list_module.html": [
          "document",
          "list",
          "list:modul",
          "modul"
        ],
        "FormPages.html": [
          "class",
          "compon",
          "creat",
          "formpag"
        ],
        "FormPages.html#trigger": [
          "call",
          "current",
          "data",
          "element",
          "event",
          "eventnam",
          "formpages#trigg",
          "function",
          "main",
          "make",
          "object",
          "page",
          "param",
          "pass",
          "proxi",
          "trigger"
        ],
        "FormPages.html#on": [
          "cb",
          "configur",
          "event",
          "eventnam",
          "filter",
          "formpages#on",
          "function",
          "plugin"
        ],
        "FormPages.html#canMoveForwards": [
          "boolean",
          "canmoveforward",
          "check",
          "formpages#canmoveforward",
          "forward",
          "function",
          "move",
          "page"
        ],
        "FormPages.html#canMoveBackwards": [
          "backward",
          "boolean",
          "canmovebackward",
          "check",
          "formpages#canmovebackward",
          "function",
          "move",
          "page"
        ],
        "FormPages.html#getTotalPages": [
          "amount",
          "check",
          "element",
          "formpages#gettotalpag",
          "function",
          "gettotalpag",
          "match",
          "number",
          "option",
          "this.options.formpageclass",
          "valu"
        ],
        "FormPages.html#goTo": [
          "allow",
          "bound",
          "can't",
          "case",
          "compon",
          "current",
          "desir",
          "form",
          "formpages#goto",
          "function",
          "goto",
          "move",
          "number",
          "out",
          "page",
          "return",
          "specif",
          "tri",
          "valid"
        ],
        "FormPages.html#goToNextPage": [
          "current",
          "form",
          "formpages#gotonextpag",
          "function",
          "gotonextpag",
          "move",
          "next",
          "number",
          "page",
          "return",
          "tri"
        ],
        "FormPages.html#goToPrevPage": [
          "current",
          "form",
          "formpages#gotoprevpag",
          "function",
          "gotoprevpag",
          "move",
          "number",
          "page",
          "previou",
          "return",
          "tri"
        ],
        "FormPages.html#getParentDimensions": [
          "dimens",
          "formpages#getparentdimens",
          "function",
          "get",
          "getparentdimens",
          "page",
          "parent'"
        ],
        "FormPages.html#getPageDimensions": [
          "adjust",
          "anim",
          "dimens",
          "formpages#getpagedimens",
          "function",
          "get",
          "getpagedimens",
          "help",
          "page",
          "pagenumb",
          "possibl"
        ],
        "FormPages.html#getCurrentPageElement": [
          "activ",
          "formpages#getcurrentpageel",
          "function",
          "get",
          "getcurrentpageel",
          "jqueri",
          "jquery|nul",
          "object",
          "page"
        ],
        "module-Utils.html": [
          "modul",
          "module:util",
          "util"
        ]
      },
      "length": 19
    },
    "tokenStore": {
      "root": {
        "docs": {},
        "f": {
          "docs": {},
          "o": {
            "docs": {},
            "r": {
              "docs": {},
              "k": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 10
                  }
                }
              },
              "m": {
                "docs": {
                  "FormPages.html#goTo": {
                    "ref": "FormPages.html#goTo",
                    "tf": 2.631578947368421
                  },
                  "FormPages.html#goToNextPage": {
                    "ref": "FormPages.html#goToNextPage",
                    "tf": 6.25
                  },
                  "FormPages.html#goToPrevPage": {
                    "ref": "FormPages.html#goToPrevPage",
                    "tf": 6.25
                  }
                },
                "p": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "g": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 600
                        },
                        "FormPages.html": {
                          "ref": "FormPages.html",
                          "tf": 1916.6666666666667
                        }
                      },
                      "e": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "#": {
                            "docs": {},
                            "t": {
                              "docs": {},
                              "r": {
                                "docs": {},
                                "i": {
                                  "docs": {},
                                  "g": {
                                    "docs": {},
                                    "g": {
                                      "docs": {
                                        "FormPages.html#trigger": {
                                          "ref": "FormPages.html#trigger",
                                          "tf": 1150
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            },
                            "o": {
                              "docs": {},
                              "n": {
                                "docs": {
                                  "FormPages.html#on": {
                                    "ref": "FormPages.html#on",
                                    "tf": 1300
                                  }
                                }
                              }
                            },
                            "c": {
                              "docs": {},
                              "a": {
                                "docs": {},
                                "n": {
                                  "docs": {},
                                  "m": {
                                    "docs": {},
                                    "o": {
                                      "docs": {},
                                      "v": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "f": {
                                            "docs": {},
                                            "o": {
                                              "docs": {},
                                              "r": {
                                                "docs": {},
                                                "w": {
                                                  "docs": {},
                                                  "a": {
                                                    "docs": {},
                                                    "r": {
                                                      "docs": {},
                                                      "d": {
                                                        "docs": {
                                                          "FormPages.html#canMoveForwards": {
                                                            "ref": "FormPages.html#canMoveForwards",
                                                            "tf": 1150
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          },
                                          "b": {
                                            "docs": {},
                                            "a": {
                                              "docs": {},
                                              "c": {
                                                "docs": {},
                                                "k": {
                                                  "docs": {},
                                                  "w": {
                                                    "docs": {},
                                                    "a": {
                                                      "docs": {},
                                                      "r": {
                                                        "docs": {},
                                                        "d": {
                                                          "docs": {
                                                            "FormPages.html#canMoveBackwards": {
                                                              "ref": "FormPages.html#canMoveBackwards",
                                                              "tf": 1150
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            },
                            "g": {
                              "docs": {},
                              "e": {
                                "docs": {},
                                "t": {
                                  "docs": {},
                                  "t": {
                                    "docs": {},
                                    "o": {
                                      "docs": {},
                                      "t": {
                                        "docs": {},
                                        "a": {
                                          "docs": {},
                                          "l": {
                                            "docs": {},
                                            "p": {
                                              "docs": {},
                                              "a": {
                                                "docs": {},
                                                "g": {
                                                  "docs": {
                                                    "FormPages.html#getTotalPages": {
                                                      "ref": "FormPages.html#getTotalPages",
                                                      "tf": 1150
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  },
                                  "p": {
                                    "docs": {},
                                    "a": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "n": {
                                            "docs": {},
                                            "t": {
                                              "docs": {},
                                              "d": {
                                                "docs": {},
                                                "i": {
                                                  "docs": {},
                                                  "m": {
                                                    "docs": {},
                                                    "e": {
                                                      "docs": {},
                                                      "n": {
                                                        "docs": {},
                                                        "s": {
                                                          "docs": {
                                                            "FormPages.html#getParentDimensions": {
                                                              "ref": "FormPages.html#getParentDimensions",
                                                              "tf": 1150
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      },
                                      "g": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "d": {
                                            "docs": {},
                                            "i": {
                                              "docs": {},
                                              "m": {
                                                "docs": {},
                                                "e": {
                                                  "docs": {},
                                                  "n": {
                                                    "docs": {},
                                                    "s": {
                                                      "docs": {
                                                        "FormPages.html#getPageDimensions": {
                                                          "ref": "FormPages.html#getPageDimensions",
                                                          "tf": 1150
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  },
                                  "c": {
                                    "docs": {},
                                    "u": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "r": {
                                          "docs": {},
                                          "e": {
                                            "docs": {},
                                            "n": {
                                              "docs": {},
                                              "t": {
                                                "docs": {},
                                                "p": {
                                                  "docs": {},
                                                  "a": {
                                                    "docs": {},
                                                    "g": {
                                                      "docs": {},
                                                      "e": {
                                                        "docs": {},
                                                        "e": {
                                                          "docs": {},
                                                          "l": {
                                                            "docs": {
                                                              "FormPages.html#getCurrentPageElement": {
                                                                "ref": "FormPages.html#getCurrentPageElement",
                                                                "tf": 1150
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              },
                              "o": {
                                "docs": {},
                                "t": {
                                  "docs": {},
                                  "o": {
                                    "docs": {
                                      "FormPages.html#goTo": {
                                        "ref": "FormPages.html#goTo",
                                        "tf": 1150
                                      }
                                    },
                                    "n": {
                                      "docs": {},
                                      "e": {
                                        "docs": {},
                                        "x": {
                                          "docs": {},
                                          "t": {
                                            "docs": {},
                                            "p": {
                                              "docs": {},
                                              "a": {
                                                "docs": {},
                                                "g": {
                                                  "docs": {
                                                    "FormPages.html#goToNextPage": {
                                                      "ref": "FormPages.html#goToNextPage",
                                                      "tf": 1150
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "p": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "v": {
                                            "docs": {},
                                            "p": {
                                              "docs": {},
                                              "a": {
                                                "docs": {},
                                                "g": {
                                                  "docs": {
                                                    "FormPages.html#goToPrevPage": {
                                                      "ref": "FormPages.html#goToPrevPage",
                                                      "tf": 1150
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                },
                "s": {
                  "docs": {},
                  "g": {
                    "docs": {},
                    "i": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "h": {
                          "docs": {},
                          "u": {
                            "docs": {},
                            "b": {
                              "docs": {
                                "index.html": {
                                  "ref": "index.html",
                                  "tf": 10
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "w": {
                "docs": {},
                "a": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "FormPages.html#canMoveForwards": {
                          "ref": "FormPages.html#canMoveForwards",
                          "tf": 12.5
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "t": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "FormPages.html#trigger": {
                            "ref": "FormPages.html#trigger",
                            "tf": 110
                          },
                          "FormPages.html#on": {
                            "ref": "FormPages.html#on",
                            "tf": 110
                          },
                          "FormPages.html#canMoveForwards": {
                            "ref": "FormPages.html#canMoveForwards",
                            "tf": 110
                          },
                          "FormPages.html#canMoveBackwards": {
                            "ref": "FormPages.html#canMoveBackwards",
                            "tf": 110
                          },
                          "FormPages.html#getTotalPages": {
                            "ref": "FormPages.html#getTotalPages",
                            "tf": 110
                          },
                          "FormPages.html#goTo": {
                            "ref": "FormPages.html#goTo",
                            "tf": 110
                          },
                          "FormPages.html#goToNextPage": {
                            "ref": "FormPages.html#goToNextPage",
                            "tf": 110
                          },
                          "FormPages.html#goToPrevPage": {
                            "ref": "FormPages.html#goToPrevPage",
                            "tf": 110
                          },
                          "FormPages.html#getParentDimensions": {
                            "ref": "FormPages.html#getParentDimensions",
                            "tf": 110
                          },
                          "FormPages.html#getPageDimensions": {
                            "ref": "FormPages.html#getPageDimensions",
                            "tf": 110
                          },
                          "FormPages.html#getCurrentPageElement": {
                            "ref": "FormPages.html#getCurrentPageElement",
                            "tf": 110
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "l": {
              "docs": {},
              "t": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "FormPages.html#on": {
                        "ref": "FormPages.html#on",
                        "tf": 33.33333333333333
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "i": {
            "docs": {},
            "t": {
              "docs": {},
              "h": {
                "docs": {},
                "u": {
                  "docs": {},
                  "b": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 10
                      }
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "t": {
              "docs": {
                "FormPages.html#getParentDimensions": {
                  "ref": "FormPages.html#getParentDimensions",
                  "tf": 12.5
                },
                "FormPages.html#getPageDimensions": {
                  "ref": "FormPages.html#getPageDimensions",
                  "tf": 7.142857142857142
                },
                "FormPages.html#getCurrentPageElement": {
                  "ref": "FormPages.html#getCurrentPageElement",
                  "tf": 10
                }
              },
              "t": {
                "docs": {},
                "o": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "l": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "g": {
                              "docs": {
                                "FormPages.html#getTotalPages": {
                                  "ref": "FormPages.html#getTotalPages",
                                  "tf": 700
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "p": {
                "docs": {},
                "a": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "d": {
                            "docs": {},
                            "i": {
                              "docs": {},
                              "m": {
                                "docs": {},
                                "e": {
                                  "docs": {},
                                  "n": {
                                    "docs": {},
                                    "s": {
                                      "docs": {
                                        "FormPages.html#getParentDimensions": {
                                          "ref": "FormPages.html#getParentDimensions",
                                          "tf": 700
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  },
                  "g": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "i": {
                          "docs": {},
                          "m": {
                            "docs": {},
                            "e": {
                              "docs": {},
                              "n": {
                                "docs": {},
                                "s": {
                                  "docs": {
                                    "FormPages.html#getPageDimensions": {
                                      "ref": "FormPages.html#getPageDimensions",
                                      "tf": 683.3333333333334
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "c": {
                "docs": {},
                "u": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "r": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "n": {
                          "docs": {},
                          "t": {
                            "docs": {},
                            "p": {
                              "docs": {},
                              "a": {
                                "docs": {},
                                "g": {
                                  "docs": {},
                                  "e": {
                                    "docs": {},
                                    "e": {
                                      "docs": {},
                                      "l": {
                                        "docs": {
                                          "FormPages.html#getCurrentPageElement": {
                                            "ref": "FormPages.html#getCurrentPageElement",
                                            "tf": 700
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "t": {
              "docs": {},
              "o": {
                "docs": {
                  "FormPages.html#goTo": {
                    "ref": "FormPages.html#goTo",
                    "tf": 683.3333333333334
                  }
                },
                "n": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "x": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "g": {
                              "docs": {
                                "FormPages.html#goToNextPage": {
                                  "ref": "FormPages.html#goToNextPage",
                                  "tf": 700
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                },
                "p": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "v": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "g": {
                              "docs": {
                                "FormPages.html#goToPrevPage": {
                                  "ref": "FormPages.html#goToPrevPage",
                                  "tf": 700
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              }
            }
          }
        },
        "p": {
          "docs": {},
          "a": {
            "docs": {},
            "g": {
              "docs": {},
              "i": {
                "docs": {},
                "n": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 10
                    }
                  }
                }
              },
              "e": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  },
                  "FormPages.html#canMoveForwards": {
                    "ref": "FormPages.html#canMoveForwards",
                    "tf": 12.5
                  },
                  "FormPages.html#canMoveBackwards": {
                    "ref": "FormPages.html#canMoveBackwards",
                    "tf": 12.5
                  },
                  "FormPages.html#goTo": {
                    "ref": "FormPages.html#goTo",
                    "tf": 41.22807017543859
                  },
                  "FormPages.html#goToNextPage": {
                    "ref": "FormPages.html#goToNextPage",
                    "tf": 12.5
                  },
                  "FormPages.html#goToPrevPage": {
                    "ref": "FormPages.html#goToPrevPage",
                    "tf": 12.5
                  },
                  "FormPages.html#getParentDimensions": {
                    "ref": "FormPages.html#getParentDimensions",
                    "tf": 12.5
                  },
                  "FormPages.html#getPageDimensions": {
                    "ref": "FormPages.html#getPageDimensions",
                    "tf": 7.142857142857142
                  },
                  "FormPages.html#getCurrentPageElement": {
                    "ref": "FormPages.html#getCurrentPageElement",
                    "tf": 10
                  }
                },
                "n": {
                  "docs": {},
                  "u": {
                    "docs": {},
                    "m": {
                      "docs": {},
                      "b": {
                        "docs": {
                          "FormPages.html#getPageDimensions": {
                            "ref": "FormPages.html#getPageDimensions",
                            "tf": 33.33333333333333
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "a": {
                "docs": {},
                "m": {
                  "docs": {
                    "FormPages.html#trigger": {
                      "ref": "FormPages.html#trigger",
                      "tf": 33.33333333333333
                    }
                  }
                }
              },
              "e": {
                "docs": {},
                "n": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "'": {
                      "docs": {
                        "FormPages.html#getParentDimensions": {
                          "ref": "FormPages.html#getParentDimensions",
                          "tf": 12.5
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "s": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "x": {
                "docs": {},
                "i": {
                  "docs": {
                    "FormPages.html#trigger": {
                      "ref": "FormPages.html#trigger",
                      "tf": 4.166666666666666
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {},
              "v": {
                "docs": {},
                "i": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "u": {
                      "docs": {
                        "FormPages.html#goToPrevPage": {
                          "ref": "FormPages.html#goToPrevPage",
                          "tf": 6.25
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "u": {
              "docs": {},
              "g": {
                "docs": {},
                "i": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "FormPages.html#on": {
                        "ref": "FormPages.html#on",
                        "tf": 16.666666666666664
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "s": {
              "docs": {},
              "s": {
                "docs": {},
                "i": {
                  "docs": {},
                  "b": {
                    "docs": {},
                    "l": {
                      "docs": {
                        "FormPages.html#getPageDimensions": {
                          "ref": "FormPages.html#getPageDimensions",
                          "tf": 7.142857142857142
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {},
              "o": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 10
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "FormPages.html#goTo": {
                        "ref": "FormPages.html#goTo",
                        "tf": 2.631578947368421
                      },
                      "FormPages.html#goToNextPage": {
                        "ref": "FormPages.html#goToNextPage",
                        "tf": 6.25
                      },
                      "FormPages.html#goToPrevPage": {
                        "ref": "FormPages.html#goToPrevPage",
                        "tf": 6.25
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 10
                    }
                  }
                }
              }
            }
          },
          "p": {
            "docs": {},
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "i": {
                  "docs": {},
                  "f": {
                    "docs": {
                      "FormPages.html#goTo": {
                        "ref": "FormPages.html#goTo",
                        "tf": 2.631578947368421
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "w": {
          "docs": {},
          "a": {
            "docs": {},
            "y": {
              "docs": {
                "index.html": {
                  "ref": "index.html",
                  "tf": 10
                }
              }
            }
          }
        },
        "d": {
          "docs": {},
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "list_class.html": {
                            "ref": "list_class.html",
                            "tf": 35
                          },
                          "list_module.html": {
                            "ref": "list_module.html",
                            "tf": 35
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "t": {
              "docs": {},
              "a": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "s": {
              "docs": {},
              "i": {
                "docs": {},
                "r": {
                  "docs": {
                    "FormPages.html#goTo": {
                      "ref": "FormPages.html#goTo",
                      "tf": 2.631578947368421
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "e": {
                "docs": {},
                "n": {
                  "docs": {},
                  "s": {
                    "docs": {
                      "FormPages.html#getParentDimensions": {
                        "ref": "FormPages.html#getParentDimensions",
                        "tf": 62.5
                      },
                      "FormPages.html#getPageDimensions": {
                        "ref": "FormPages.html#getPageDimensions",
                        "tf": 40.47619047619047
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "e": {
          "docs": {},
          "v": {
            "docs": {},
            "e": {
              "docs": {},
              "n": {
                "docs": {},
                "t": {
                  "docs": {
                    "undefined": {
                      "tf": 950
                    },
                    "FormPages.html#trigger": {
                      "ref": "FormPages.html#trigger",
                      "tf": 8.333333333333332
                    },
                    "FormPages.html#on": {
                      "ref": "FormPages.html#on",
                      "tf": 16.666666666666664
                    }
                  },
                  "n": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "m": {
                        "docs": {
                          "FormPages.html#trigger": {
                            "ref": "FormPages.html#trigger",
                            "tf": 33.33333333333333
                          },
                          "FormPages.html#on": {
                            "ref": "FormPages.html#on",
                            "tf": 33.33333333333333
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "e": {
              "docs": {},
              "m": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "FormPages.html#trigger": {
                          "ref": "FormPages.html#trigger",
                          "tf": 4.166666666666666
                        },
                        "FormPages.html#getTotalPages": {
                          "ref": "FormPages.html#getTotalPages",
                          "tf": 7.142857142857142
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "h": {
          "docs": {},
          "a": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {},
                "l": {
                  "docs": {
                    "undefined": {
                      "tf": 950
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "l": {
              "docs": {},
              "p": {
                "docs": {
                  "FormPages.html#getPageDimensions": {
                    "ref": "FormPages.html#getPageDimensions",
                    "tf": 7.142857142857142
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "o": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "undefined": {
                        "tf": 110
                      },
                      "list_tutorial.html": {
                        "ref": "list_tutorial.html",
                        "tf": 635
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "i": {
              "docs": {
                "FormPages.html#goTo": {
                  "ref": "FormPages.html#goTo",
                  "tf": 2.631578947368421
                },
                "FormPages.html#goToNextPage": {
                  "ref": "FormPages.html#goToNextPage",
                  "tf": 6.25
                },
                "FormPages.html#goToPrevPage": {
                  "ref": "FormPages.html#goToPrevPage",
                  "tf": 6.25
                }
              },
              "g": {
                "docs": {},
                "g": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "FormPages.html#trigger": {
                          "ref": "FormPages.html#trigger",
                          "tf": 683.3333333333334
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "i": {
              "docs": {},
              "s": {
                "docs": {},
                ".": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "p": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "i": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "n": {
                              "docs": {},
                              "s": {
                                "docs": {},
                                ".": {
                                  "docs": {},
                                  "f": {
                                    "docs": {},
                                    "o": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "m": {
                                          "docs": {},
                                          "p": {
                                            "docs": {},
                                            "a": {
                                              "docs": {},
                                              "g": {
                                                "docs": {},
                                                "e": {
                                                  "docs": {},
                                                  "c": {
                                                    "docs": {},
                                                    "l": {
                                                      "docs": {},
                                                      "a": {
                                                        "docs": {},
                                                        "s": {
                                                          "docs": {},
                                                          "s": {
                                                            "docs": {
                                                              "FormPages.html#getTotalPages": {
                                                                "ref": "FormPages.html#getTotalPages",
                                                                "tf": 7.142857142857142
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "a": {
          "docs": {},
          "v": {
            "docs": {},
            "a": {
              "docs": {},
              "i": {
                "docs": {},
                "l": {
                  "docs": {
                    "list_tutorial.html": {
                      "ref": "list_tutorial.html",
                      "tf": 35
                    }
                  }
                }
              }
            }
          },
          "m": {
            "docs": {},
            "o": {
              "docs": {},
              "u": {
                "docs": {},
                "n": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "FormPages.html#getTotalPages": {
                        "ref": "FormPages.html#getTotalPages",
                        "tf": 7.142857142857142
                      }
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "l": {
              "docs": {},
              "o": {
                "docs": {},
                "w": {
                  "docs": {
                    "FormPages.html#goTo": {
                      "ref": "FormPages.html#goTo",
                      "tf": 2.631578947368421
                    }
                  }
                }
              }
            }
          },
          "d": {
            "docs": {},
            "j": {
              "docs": {},
              "u": {
                "docs": {},
                "s": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "FormPages.html#getPageDimensions": {
                        "ref": "FormPages.html#getPageDimensions",
                        "tf": 7.142857142857142
                      }
                    }
                  }
                }
              }
            }
          },
          "n": {
            "docs": {},
            "i": {
              "docs": {},
              "m": {
                "docs": {
                  "FormPages.html#getPageDimensions": {
                    "ref": "FormPages.html#getPageDimensions",
                    "tf": 7.142857142857142
                  }
                }
              }
            }
          },
          "c": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "v": {
                  "docs": {
                    "FormPages.html#getCurrentPageElement": {
                      "ref": "FormPages.html#getCurrentPageElement",
                      "tf": 10
                    }
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_tutorial.html": {
                    "ref": "list_tutorial.html",
                    "tf": 110
                  },
                  "list_class.html": {
                    "ref": "list_class.html",
                    "tf": 110
                  },
                  "list_module.html": {
                    "ref": "list_module.html",
                    "tf": 110
                  }
                },
                ":": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "r": {
                            "docs": {},
                            "i": {
                              "docs": {
                                "list_tutorial.html": {
                                  "ref": "list_tutorial.html",
                                  "tf": 1300
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  },
                  "c": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "a": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "list_class.html": {
                                "ref": "list_class.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  },
                  "m": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "u": {
                          "docs": {},
                          "l": {
                            "docs": {
                              "list_module.html": {
                                "ref": "list_module.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {},
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "list_class.html": {
                      "ref": "list_class.html",
                      "tf": 635
                    },
                    "FormPages.html": {
                      "ref": "FormPages.html",
                      "tf": 110
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "FormPages.html": {
                        "ref": "FormPages.html",
                        "tf": 16.666666666666664
                      },
                      "FormPages.html#goTo": {
                        "ref": "FormPages.html#goTo",
                        "tf": 2.631578947368421
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "f": {
                "docs": {},
                "i": {
                  "docs": {},
                  "g": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "r": {
                        "docs": {
                          "FormPages.html#on": {
                            "ref": "FormPages.html#on",
                            "tf": 16.666666666666664
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "FormPages.html": {
                      "ref": "FormPages.html",
                      "tf": 16.666666666666664
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "m": {
                "docs": {},
                "o": {
                  "docs": {},
                  "v": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "f": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "r": {
                            "docs": {},
                            "w": {
                              "docs": {},
                              "a": {
                                "docs": {},
                                "r": {
                                  "docs": {},
                                  "d": {
                                    "docs": {
                                      "FormPages.html#canMoveForwards": {
                                        "ref": "FormPages.html#canMoveForwards",
                                        "tf": 700
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "b": {
                        "docs": {},
                        "a": {
                          "docs": {},
                          "c": {
                            "docs": {},
                            "k": {
                              "docs": {},
                              "w": {
                                "docs": {},
                                "a": {
                                  "docs": {},
                                  "r": {
                                    "docs": {},
                                    "d": {
                                      "docs": {
                                        "FormPages.html#canMoveBackwards": {
                                          "ref": "FormPages.html#canMoveBackwards",
                                          "tf": 700
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "'": {
                "docs": {},
                "t": {
                  "docs": {
                    "FormPages.html#goTo": {
                      "ref": "FormPages.html#goTo",
                      "tf": 2.631578947368421
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "e": {
                "docs": {
                  "FormPages.html#goTo": {
                    "ref": "FormPages.html#goTo",
                    "tf": 2.631578947368421
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "r": {
              "docs": {},
              "r": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "FormPages.html#trigger": {
                          "ref": "FormPages.html#trigger",
                          "tf": 4.166666666666666
                        },
                        "FormPages.html#goTo": {
                          "ref": "FormPages.html#goTo",
                          "tf": 2.631578947368421
                        },
                        "FormPages.html#goToNextPage": {
                          "ref": "FormPages.html#goToNextPage",
                          "tf": 6.25
                        },
                        "FormPages.html#goToPrevPage": {
                          "ref": "FormPages.html#goToPrevPage",
                          "tf": 6.25
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "b": {
            "docs": {
              "FormPages.html#on": {
                "ref": "FormPages.html#on",
                "tf": 33.33333333333333
              }
            }
          },
          "h": {
            "docs": {},
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "k": {
                  "docs": {
                    "FormPages.html#canMoveForwards": {
                      "ref": "FormPages.html#canMoveForwards",
                      "tf": 12.5
                    },
                    "FormPages.html#canMoveBackwards": {
                      "ref": "FormPages.html#canMoveBackwards",
                      "tf": 12.5
                    },
                    "FormPages.html#getTotalPages": {
                      "ref": "FormPages.html#getTotalPages",
                      "tf": 7.142857142857142
                    }
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "o": {
            "docs": {},
            "d": {
              "docs": {},
              "u": {
                "docs": {},
                "l": {
                  "docs": {
                    "list_module.html": {
                      "ref": "list_module.html",
                      "tf": 635
                    },
                    "module-Utils.html": {
                      "ref": "module-Utils.html",
                      "tf": 110
                    }
                  },
                  "e": {
                    "docs": {},
                    ":": {
                      "docs": {},
                      "u": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "i": {
                            "docs": {},
                            "l": {
                              "docs": {
                                "module-Utils.html": {
                                  "ref": "module-Utils.html",
                                  "tf": 1300
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "v": {
              "docs": {},
              "e": {
                "docs": {
                  "FormPages.html#canMoveForwards": {
                    "ref": "FormPages.html#canMoveForwards",
                    "tf": 12.5
                  },
                  "FormPages.html#canMoveBackwards": {
                    "ref": "FormPages.html#canMoveBackwards",
                    "tf": 12.5
                  },
                  "FormPages.html#goTo": {
                    "ref": "FormPages.html#goTo",
                    "tf": 7.894736842105263
                  },
                  "FormPages.html#goToNextPage": {
                    "ref": "FormPages.html#goToNextPage",
                    "tf": 6.25
                  },
                  "FormPages.html#goToPrevPage": {
                    "ref": "FormPages.html#goToPrevPage",
                    "tf": 6.25
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "i": {
              "docs": {},
              "n": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  }
                }
              }
            },
            "k": {
              "docs": {},
              "e": {
                "docs": {
                  "FormPages.html#trigger": {
                    "ref": "FormPages.html#trigger",
                    "tf": 4.166666666666666
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "c": {
                "docs": {},
                "h": {
                  "docs": {
                    "FormPages.html#getTotalPages": {
                      "ref": "FormPages.html#getTotalPages",
                      "tf": 7.142857142857142
                    }
                  }
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "b": {
            "docs": {},
            "j": {
              "docs": {},
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "FormPages.html#trigger": {
                        "ref": "FormPages.html#trigger",
                        "tf": 4.166666666666666
                      },
                      "FormPages.html#getCurrentPageElement": {
                        "ref": "FormPages.html#getCurrentPageElement",
                        "tf": 10
                      }
                    }
                  }
                }
              }
            }
          },
          "p": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "FormPages.html#getTotalPages": {
                        "ref": "FormPages.html#getTotalPages",
                        "tf": 7.142857142857142
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {
                "FormPages.html#goTo": {
                  "ref": "FormPages.html#goTo",
                  "tf": 2.631578947368421
                }
              }
            }
          }
        },
        "b": {
          "docs": {},
          "o": {
            "docs": {},
            "o": {
              "docs": {},
              "l": {
                "docs": {},
                "e": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "n": {
                      "docs": {
                        "FormPages.html#canMoveForwards": {
                          "ref": "FormPages.html#canMoveForwards",
                          "tf": 50
                        },
                        "FormPages.html#canMoveBackwards": {
                          "ref": "FormPages.html#canMoveBackwards",
                          "tf": 50
                        }
                      }
                    }
                  }
                }
              }
            },
            "u": {
              "docs": {},
              "n": {
                "docs": {},
                "d": {
                  "docs": {
                    "FormPages.html#goTo": {
                      "ref": "FormPages.html#goTo",
                      "tf": 2.631578947368421
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "c": {
              "docs": {},
              "k": {
                "docs": {},
                "w": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "r": {
                      "docs": {},
                      "d": {
                        "docs": {
                          "FormPages.html#canMoveBackwards": {
                            "ref": "FormPages.html#canMoveBackwards",
                            "tf": 12.5
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "n": {
          "docs": {},
          "u": {
            "docs": {},
            "m": {
              "docs": {},
              "b": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "FormPages.html#getTotalPages": {
                        "ref": "FormPages.html#getTotalPages",
                        "tf": 50
                      },
                      "FormPages.html#goTo": {
                        "ref": "FormPages.html#goTo",
                        "tf": 33.33333333333333
                      },
                      "FormPages.html#goToNextPage": {
                        "ref": "FormPages.html#goToNextPage",
                        "tf": 50
                      },
                      "FormPages.html#goToPrevPage": {
                        "ref": "FormPages.html#goToPrevPage",
                        "tf": 50
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "x": {
              "docs": {},
              "t": {
                "docs": {
                  "FormPages.html#goToNextPage": {
                    "ref": "FormPages.html#goToNextPage",
                    "tf": 6.25
                  }
                }
              }
            }
          }
        },
        "v": {
          "docs": {},
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "u": {
                "docs": {
                  "FormPages.html#getTotalPages": {
                    "ref": "FormPages.html#getTotalPages",
                    "tf": 7.142857142857142
                  }
                }
              },
              "i": {
                "docs": {},
                "d": {
                  "docs": {
                    "FormPages.html#goTo": {
                      "ref": "FormPages.html#goTo",
                      "tf": 2.631578947368421
                    }
                  }
                }
              }
            }
          }
        },
        "j": {
          "docs": {},
          "q": {
            "docs": {},
            "u": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "FormPages.html#getCurrentPageElement": {
                        "ref": "FormPages.html#getCurrentPageElement",
                        "tf": 10
                      }
                    }
                  },
                  "y": {
                    "docs": {},
                    "|": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "u": {
                          "docs": {},
                          "l": {
                            "docs": {
                              "FormPages.html#getCurrentPageElement": {
                                "ref": "FormPages.html#getCurrentPageElement",
                                "tf": 50
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "t": {
            "docs": {},
            "i": {
              "docs": {},
              "l": {
                "docs": {
                  "module-Utils.html": {
                    "ref": "module-Utils.html",
                    "tf": 600
                  }
                }
              }
            }
          }
        }
      },
      "length": 153
    },
    "corpusTokens": [
      "activ",
      "adjust",
      "allow",
      "amount",
      "anim",
      "avail",
      "backward",
      "boolean",
      "bound",
      "call",
      "can't",
      "canmovebackward",
      "canmoveforward",
      "case",
      "cb",
      "check",
      "class",
      "compon",
      "configur",
      "creat",
      "current",
      "data",
      "desir",
      "dimens",
      "document",
      "element",
      "event",
      "eventnam",
      "filter",
      "fork",
      "form",
      "formpag",
      "formpages#canmovebackward",
      "formpages#canmoveforward",
      "formpages#getcurrentpageel",
      "formpages#getpagedimens",
      "formpages#getparentdimens",
      "formpages#gettotalpag",
      "formpages#goto",
      "formpages#gotonextpag",
      "formpages#gotoprevpag",
      "formpages#on",
      "formpages#trigg",
      "formsgithub",
      "forward",
      "function",
      "get",
      "getcurrentpageel",
      "getpagedimens",
      "getparentdimens",
      "gettotalpag",
      "github",
      "global",
      "goto",
      "gotonextpag",
      "gotoprevpag",
      "handl",
      "help",
      "index",
      "jqueri",
      "jquery|nul",
      "list",
      "list:class",
      "list:modul",
      "list:tutori",
      "main",
      "make",
      "match",
      "modul",
      "module:util",
      "move",
      "next",
      "number",
      "object",
      "option",
      "out",
      "page",
      "pagenumb",
      "pagin",
      "param",
      "parent'",
      "pass",
      "plugin",
      "possibl",
      "previou",
      "proxi",
      "readm",
      "repo",
      "return",
      "simpl",
      "specif",
      "this.options.formpageclass",
      "tri",
      "trigger",
      "tutori",
      "util",
      "valid",
      "valu",
      "way"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "FormPages",
      "longname": "index",
      "name": "FormPages",
      "tags": "index",
      "summary": "A simple way to paginate your formsGitHub repo Fork us on GitHub",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "undefined": {
      "kind": "tutorial",
      "title": "Event Handling",
      "longname": "event-handling",
      "name": "event-handling",
      "tags": "event-handling",
      "summary": "",
      "description": "",
      "body": ""
    },
    "list_tutorial.html": {
      "id": "list_tutorial.html",
      "kind": "list",
      "title": "Tutorials",
      "longname": "list:tutorial",
      "name": "Tutorials",
      "tags": "list:tutorial",
      "summary": "All available tutorials.",
      "description": "",
      "body": ""
    },
    "list_class.html": {
      "id": "list_class.html",
      "kind": "list",
      "title": "Classes",
      "longname": "list:class",
      "name": "Classes",
      "tags": "list:class",
      "summary": "All documented classes.",
      "description": "",
      "body": ""
    },
    "list_module.html": {
      "id": "list_module.html",
      "kind": "list",
      "title": "Modules",
      "longname": "list:module",
      "name": "Modules",
      "tags": "list:module",
      "summary": "All documented modules.",
      "description": "",
      "body": ""
    },
    "FormPages.html": {
      "id": "FormPages.html",
      "kind": "class",
      "title": "FormPages",
      "longname": "FormPages",
      "name": "FormPages",
      "tags": "FormPages",
      "summary": "",
      "description": "Creates the FormPages component.",
      "body": ""
    },
    "FormPages.html#trigger": {
      "id": "FormPages.html#trigger",
      "kind": "function",
      "title": "trigger( eventName, params )",
      "longname": "FormPages#trigger",
      "name": "trigger",
      "tags": "FormPages#trigger trigger",
      "summary": "",
      "description": "Makes a proxy and calls events to the main $element object, passing the current page as event data."
    },
    "FormPages.html#on": {
      "id": "FormPages.html#on",
      "kind": "function",
      "title": "on( eventName, cb, filter )",
      "longname": "FormPages#on",
      "name": "on",
      "tags": "FormPages#on on",
      "summary": "",
      "description": "Configures events to the plugin"
    },
    "FormPages.html#canMoveForwards": {
      "id": "FormPages.html#canMoveForwards",
      "kind": "function",
      "title": "canMoveForwards() → {boolean}",
      "longname": "FormPages#canMoveForwards",
      "name": "canMoveForwards",
      "tags": "FormPages#canMoveForwards canMoveForwards",
      "summary": "",
      "description": "Checks if the pages can move forwards."
    },
    "FormPages.html#canMoveBackwards": {
      "id": "FormPages.html#canMoveBackwards",
      "kind": "function",
      "title": "canMoveBackwards() → {boolean}",
      "longname": "FormPages#canMoveBackwards",
      "name": "canMoveBackwards",
      "tags": "FormPages#canMoveBackwards canMoveBackwards",
      "summary": "",
      "description": "Checks if the pages can move backwards."
    },
    "FormPages.html#getTotalPages": {
      "id": "FormPages.html#getTotalPages",
      "kind": "function",
      "title": "getTotalPages() → {number}",
      "longname": "FormPages#getTotalPages",
      "name": "getTotalPages",
      "tags": "FormPages#getTotalPages getTotalPages",
      "summary": "",
      "description": "Checks the amount of the elements that matches to the this.options.formPageClass option value."
    },
    "FormPages.html#goTo": {
      "id": "FormPages.html#goTo",
      "kind": "function",
      "title": "goTo( page ) → {number}",
      "longname": "FormPages#goTo",
      "name": "goTo",
      "tags": "FormPages#goTo goTo",
      "summary": "",
      "description": "Tries to move the form to a specific page. This also validates if the move is allowed (not out of bounds). In case the component can't move to the desired page, it returns the current page."
    },
    "FormPages.html#goToNextPage": {
      "id": "FormPages.html#goToNextPage",
      "kind": "function",
      "title": "goToNextPage() → {number}",
      "longname": "FormPages#goToNextPage",
      "name": "goToNextPage",
      "tags": "FormPages#goToNextPage goToNextPage",
      "summary": "",
      "description": "Tries to move the form to the next page and returns the current page."
    },
    "FormPages.html#goToPrevPage": {
      "id": "FormPages.html#goToPrevPage",
      "kind": "function",
      "title": "goToPrevPage() → {number}",
      "longname": "FormPages#goToPrevPage",
      "name": "goToPrevPage",
      "tags": "FormPages#goToPrevPage goToPrevPage",
      "summary": "",
      "description": "Tries to move the form to the previous page and returns the current page."
    },
    "FormPages.html#getParentDimensions": {
      "id": "FormPages.html#getParentDimensions",
      "kind": "function",
      "title": "getParentDimensions() → {Dimensions}",
      "longname": "FormPages#getParentDimensions",
      "name": "getParentDimensions",
      "tags": "FormPages#getParentDimensions getParentDimensions",
      "summary": "",
      "description": "Gets the pages' parent's dimensions."
    },
    "FormPages.html#getPageDimensions": {
      "id": "FormPages.html#getPageDimensions",
      "kind": "function",
      "title": "getPageDimensions( pageNumber ) → {Dimensions}",
      "longname": "FormPages#getPageDimensions",
      "name": "getPageDimensions",
      "tags": "FormPages#getPageDimensions getPageDimensions",
      "summary": "",
      "description": "Gets the dimensions of the page to help adjust possible animations."
    },
    "FormPages.html#getCurrentPageElement": {
      "id": "FormPages.html#getCurrentPageElement",
      "kind": "function",
      "title": "getCurrentPageElement() → {jQuery|null}",
      "longname": "FormPages#getCurrentPageElement",
      "name": "getCurrentPageElement",
      "tags": "FormPages#getCurrentPageElement getCurrentPageElement",
      "summary": "",
      "description": "Gets the active page as jQuery object."
    },
    "module-Utils.html": {
      "id": "module-Utils.html",
      "kind": "module",
      "title": "Utils",
      "longname": "module:Utils",
      "name": "Utils",
      "tags": "module:Utils",
      "summary": "",
      "description": "",
      "body": ""
    }
  }
};